...
title: v11.6.3100 User Guide - Use Dynamsoft Barcode Reader JavaScript Edition In Framework
...
2. Install DBR-JS SDK with the following command:
    ...
    ```sh
    npm install dynamsoft-barcode-reader-bundle@11.6.3100 -E
    ```
...
3. Confirm the installation by checking the `package.json`. You should see:
    ...
    ```json
    {
      ...
      "dependencies": {
        ...
        "dynamsoft-barcode-reader-bundle": "11.6.3100"
      }
    }
    ```

Notice that there is no `^` before `11.6.3100`. No `^` indicates an exact version, ensuring stability and avoiding automatic upgrades even without `package-lock.json`.
