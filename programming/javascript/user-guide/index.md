...
title: DBR JS - v9.2.12 User Guide
...
- Build the UI element from scratch and connect it to the SDK with the API `setUIElement(HTMLElement)`.
  ...
  - Embed the video
    ...
    > The video element will be created and appended to the DIV element with the class name `dce-video-container` , make sure the class name is the same. Besides, the CSS property `position` of the DIV element must be either `relative` , `absolute` , `fixed` , or `sticky` .
