...
- **Decode dense grid barcodes more completely** - Added [`LayoutAnalyzer`]({{ site.dbr_js_api }}utility/layout-analyzer.html) to organize barcode locations into logical line or matrix layouts and infer unrecognized barcode regions when gaps exist, enabling workflows such as fast first-pass decoding, missing-region inference, and targeted second-pass decoding on dense N*M barcode layouts.

### New

- Added [`LayoutAnalyzer`]({{ site.dbr_js_api }}utility/layout-analyzer.html) class with [`analyze()`]({{ site.dbr_js_api }}utility/layout-analyzer.html#analyze) static method for quadrilateral layout analysis.

- Added [`EnumLayoutPattern`]({{ site.dbr_js_api }}utility/enum-layout-pattern.html) enumeration with values `LP_UNKNOWN`, `LP_LINES`, and `LP_MATRIX`.

- Added [`EnumLayoutElementSource`]({{ site.dbr_js_api }}utility/enum-layout-element-source.html) enumeration with values `LES_NONE`, `LES_INPUT`, and `LES_INFERRED`.

- Added [`LayoutAxis`]({{ site.dbr_js_api }}utility/layout-axis.html), [`LayoutAnalysisParameter`]({{ site.dbr_js_api }}utility/layout-analysis-parameter.html), [`LayoutElement`]({{ site.dbr_js_api }}utility/layout-element.html), and [`LayoutAnalysisResult`]({{ site.dbr_js_api }}utility/layout-analysis-result.html) for layout analysis configuration and results.
