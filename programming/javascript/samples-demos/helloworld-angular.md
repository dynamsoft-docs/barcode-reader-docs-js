...
> * To release resources timely, the `BarcodeScanner` instance is destroyed with the component.
> * The method `setUIElement()` specifies the UI with the native element in `barcode-scanner.component.html` which we just copied over in the previous step.
...
If you followed all the steps correctly, you will have a working page that turns one of the cameras hooked to or built in your computer into a barcode scanner. However, the found barcodes are not displayed anywhere yet. At the same time, there is a short delay for the initialization of the library which doesn't seem user-friendly. The following steps take care of these two issues.
...
* Add CSS rules as well in `hello-world.component.css`
...
* Add a `EventEmitter` in `barcode-scanner.component.ts` and use the event `onFrameRead` to return the results
